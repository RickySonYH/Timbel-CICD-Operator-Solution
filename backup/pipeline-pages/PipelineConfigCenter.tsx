...
const PipelineConfigCenter: React.FC = () => {
  ...
  const loadPipelineData = async () => {
    try {
      ...
      console.log('🔍 PipelineConfigCenter - API 요청 시작:', { authToken: authToken ? '있음' : '없음' });
      
      const [jobsRes, webhooksRes] = await Promise.all([
        fetch('/api/jenkins/jobs', {
          headers: { 'Authorization': `Bearer ${authToken}` }
        }),
        fetch('/api/github/webhooks', {
          headers: { 'Authorization': `Bearer ${authToken}` }
        })
      ]);
      ...
      // 파이프라인 템플릿은 현재 빈 배열로 설정 (실제 API가 없음)
      setPipelines([]);
  ...
  const handleCreatePipeline = async () => {
    try {
      ...
      // 1단계: Jenkins Job 생성
      const jenkinsResponse = await fetch('http://localhost:3001/api/cicd/jenkins/create-job', {
        method: 'POST',
        headers: {
          ...
          <Card>
            <CardContent sx={{ textAlign: 'center', py: 8 }}>
              ...
              <Typography variant="body2" color="text.secondary" paragraph>
                새로운 파이프라인을 생성하여 CI/CD 워크플로우를 시작하세요.
              </Typography>
              <Button
                variant="contained"
                onClick={() => setWizardOpen(true)}
